...
namespace dbu.DAL
{
    public class DBHelper
    {
        ...
        /// <returns></returns>
        public static string GetDBFieldInfoSQL(bool isAllTab, string tabName, List<string> types = null)
        {
...
from syscolumns a left join systypes b on a.xusertype=b.xusertype
    ...
    (
		...
		FROM  sys.index_columns idxCol
			left join sys.indexes idx ON (idx.object_id = idxCol.object_id  AND idx.index_id = idxCol.index_id and idx.is_unique_constraint = 1)
...
where " + (isAllTab ? "a.id in (select id from sysobjects where xtype in (" + type + "))" : "a.id=OBJECT_ID('" + tabName + "')") + 
        ...
        /// <returns></returns>
        public static Dictionary<string, List<StObjField>> GetAllTableFieldInfo(string serverAddress, string dbName, string userName, string passWord,List<string >types ,string reStr="_",params string[] replyStr)
        {

            Dictionary<string, List<StObjField>> res =new Dictionary<string, List<StObjField>>();
            ...
            try
            {
                ...
                con.Open();
                 sql = GetDBFieldInfoSQL(true, "",types);
  ...
  sys.check_constraints chk
    JOIN sys.tables tab
      ON (chk.parent_object_id = tab.object_id)";
        ...
        /// <param name="passWord">密码</param>
        /// <returns></returns>
        private static Dictionary<string, StTable> GetAllTableFieldInfo2(string serverAddress, string dbName, string userName, string passWord, List<string> types, string reStr = "_", params string[] replyStr)
        {
        ...
        /// <param name="passWord">密码</param>
        /// <returns></returns>
        private static List<StObjField> GetTableFieldInfo2(string serverAddress, string dbName, string tabName, string userName, string passWord)
        {
        ...
        /// <returns></returns>
        public static bool TextLineToServer(string serverAddress, string userName, string passWord, int maxSleepTime = 9000)
        {
