            ...
            try
            {
                ...
                foreach (string tabName in fss.Keys)
                {
                    ...
                    foreach (var f in fs)
                    {
                        ...
                        if (f.IsPrimaryKey)
                        {
                            if (!_primaryKeyIsExists) _primaryKey = f;
                    ...
                    {
                        StObjField t = fs.Where(i => i.IsIdentity).FirstOrDefault();
                        if (t != null)
                        {
                            _primaryKey = t;
                        }
                        ...
                        {
                            t = fs.Where(i => i.IsUnique).FirstOrDefault();
                            if (t != null)
                            {
                                _primaryKey = t;
                            }
                    ...
                    */
                    //拼接sql语句
                    sb.AppendLine("\t\tprivate " + staticText + " DbHelper db=new DbHelper();");
                    sb.AppendLine();
                    sb.AppendLine("\t\tprivate " + staticText + " string tabName = \"" + tabName + "\";");


                    sb.AppendLine("\t\t#region CommandText");
                    sb.AppendLine("\t\tprivate " + staticText + " string " + selectAllText + " = \"select " + string.Join(",", _notAt) + " from " + tabName + "\";");
                    sb.AppendLine("\t\tprivate " + staticText + " string " + selectByKeyText + " = \"select " + string.Join(",", _notAt) + " from " + tabName + " where " + _primaryKey.ColName + "=@" + _primaryKey.ColName + "\";");
                    sb.AppendLine("\t\tprivate " + staticText + " string " + selectByWhereText + " = \"select " + string.Join(",", _notAt) + " from " + tabName + " {0}\";");
                    sb.AppendLine("\t\t");
                    sb.AppendLine("\t\tprivate " + staticText + " string " + insertText + " = \"insert into " + tabName + "(" + string.Join(", ", _notAt_NotIdentity) + ") values(" + string.Join(", ", _AT_NotIdentity) + ");select @@IDENTITY\";");
                    sb.AppendLine("\t\tprivate " + staticText + " string " + updateByKeyText + " = \"update  " + tabName + " set " + string.Join(", ", _notAtAnd_at_NotIdentity) + " where " + _primaryKey.ColName + "=@" + firstUpperOrLower(_primaryKey.ColName) + "1\";");
                    sb.AppendLine();
                    sb.AppendLine("\t\tprivate " + staticText + " string " + deleteByKeyText + " = \"delete from " + tabName + " where " + string.Format("{0}=@{0}", _primaryKey.ColName) + "\";");
                    sb.AppendLine("\t\tprivate " + staticText + " string " + deleteByKeyCheckedText + " = \"delete from " + tabName + " where " + _primaryKey.ColName + " in ({0})\";");
                ...
                public static DataTable GetPageData_NotIn(string querystring , string where, int take, int skip, string orderString, out int total, string tableSpecialColumn = "")
                {
                    ...
                    return ds.Tables[0];
                        ...
                        if (_primaryKey.IsIdentity || _primaryKey.IsPrimaryKey || _primaryKey.IsUnique)
                        {
                            ...
                            sb.AppendLine("\t\t/// <param name=\"groupString\">分组字段  前面不加group by</param>");
                            sb.AppendLine("\t\t/// <param name=\"orderString\">排序字段  前面不加order by</param>");
                            sb.AppendLine("\t\t/// <returns></returns>");
                            sb.AppendLine("\t\tpublic " + staticText + " DataTable GetPageData_NotIn(string querystring , string where, int take, int skip, string groupString, string orderString)");
                            sb.AppendLine("\t\t{");
                            sb.AppendLine("\t\t\tstring sql = db.GetPading(tabName, querystring, \"" + k + "\", where, take, skip, groupString, orderString, PaddingType.NotIn);");
                            sb.AppendLine("\t\t\tDataTable dt = db.ExecuteDataTable(sql, CommandType.Text, null);");
                            ...
                            sb.AppendLine("\t\t/// <param name=\"orderString\">排序字段  前面不加order by</param>");
                            sb.AppendLine("\t\t/// <returns></returns>");
                            sb.AppendLine("\t\tpublic " + staticText + " DataTable GetPageData_MaxID(string querystring , string where, int take, int skip, string groupString, string orderString)");
                            sb.AppendLine("\t\t{");
                            sb.AppendLine("\t\t\tstring sql = db.GetPading(tabName, querystring, \"" + k + "\", where, take, skip, groupString, orderString, PaddingType.MaxID);");
                            sb.AppendLine("\t\t\tDataTable dt = db.ExecuteDataTable(sql, CommandType.Text, null);");
                            sb.AppendLine("\t\t\treturn dt;");
                            sb.AppendLine("\t\t}");
                            sb.AppendLine("\t\t#endregion");
                        ...
                        sb.AppendLine("\t\t/// <param name=\"orderString\">排序字段  前面不加order by</param>");
                        sb.AppendLine("\t\t/// <returns></returns>");
                        sb.AppendLine("\t\tpublic " + staticText + " DataTable GetPageData_RowNumber3(string querystring , string where, int take, int skip, string groupString, string orderString)");
                        sb.AppendLine("\t\t{");
                        //RowNumber3方式不用穿主键或者唯一约束列名
                        sb.AppendLine("\t\t\tstring sql = db.GetPading(tabName, querystring, \"\", where, take, skip, groupString, orderString, PaddingType.RowNumber3);");
                        sb.AppendLine("\t\t\tDataTable dt = db.ExecuteDataTable(sql, CommandType.Text, null);");
                        sb.AppendLine("\t\t\treturn dt;");
                        sb.AppendLine("\t\t}");
                        sb.AppendLine("\t\t#endregion");
                    ...
                    public static int Insert(Categories categories)
                    {
                        ...
                        else
                        {
                            ...
                            sb.AppendLine("\t\t/// <param name=\"" + tab_firstLower + "\">需要插入的对象</param>");
                            sb.AppendLine("\t\t/// <returns></returns>");
                            sb.AppendLine("\t\tpublic " + staticText + " int Add(" + tab_firstUp + " " + tab_firstLower + ")");
                            sb.AppendLine("\t\t{");
                ...
                */
                        ...
                        if (m_i > 0)
                        {
                            sb.AppendLine("\t\t\t\t\t,new SqlParameter(\"@" + firstUpperOrLower(_primaryKey.ColName) + "1\",SqlDbType." + TypeManager.DBTypeToSqlDbType(_primaryKey.ColTypeName) + "," + _primaryKey.OldLength + "){Value=" + tab_firstLower + "." + firstUpperOrLower(_primaryKey.ColName) + "}");
                        }
                        sb.AppendLine("\t\t\t\t});");
                        sb.AppendLine("\t\t}");
                        sb.AppendLine("\t\t#endregion");
                ...
                private static Dictionary<int, Categories> Find(string text, CommandType type, SqlParameter[] pars)
                {
                    ...
                    return categoriess;
                        ...
                        sb.AppendLine("\t\t/// <returns></returns>");
                        sb.AppendLine("\t\tpublic " + staticText + " List<" + tab_firstUp + "> Find(string text,CommandType type,params SqlParameter[] pars)");
                        sb.AppendLine("\t\t{");
                    ...
                    sb.AppendLine("");
                    sb.AppendLine("\t\tprivate " + staticText + " List<string> cols = new List<string>() { " + _cols + " };");
                    sb.AppendLine("");
        ...
        public static List<string> CreateBLL(string dicPath, string spaceName, string modelNameSpace, string dalNameSpace, Dictionary<string, List<StObjField>> fss)
        {
            try
            {
                ...
                foreach (string tabName in fss.Keys)
                {
                    ...
                    if (staticText.IndexOf("static") < 0)
                    {
                        sb.AppendLine("\t\t" + dalName + " _dal = new " + dalName + "();");
                        dalName = "_dal";
                    }
                ...
                */
                        ...
                        if (_primaryKey.IsIdentity || _primaryKey.IsPrimaryKey || _primaryKey.IsUnique)
                        {
                            ...
                            sb.AppendLine("\t\tpublic " + staticText + " DataTable GetPageData_NotIn(string querystring , string where, int take, int skip, string groupString, string orderString)");
                            sb.AppendLine("\t\t{");
                            sb.AppendLine("\t\t\treturn " + dalName + ".GetPageData_NotIn( querystring , where, take, skip, groupString, orderString);");
                            sb.AppendLine("\t\t}");
                            sb.AppendLine("\t\t#endregion");
                            sb.AppendLine("\t\t");
                            ...
                            sb.AppendLine("\t\tpublic " + staticText + " DataTable GetPageData_MaxID(string querystring , string where, int take, int skip, string groupString, string orderString)");
                            sb.AppendLine("\t\t{");
                            sb.AppendLine("\t\t\treturn " + dalName + ".GetPageData_MaxID( querystring , where, take, skip, groupString, orderString);");
                            sb.AppendLine("\t\t}");
                            sb.AppendLine("\t\t#endregion");
                            sb.AppendLine("\t\t");
                        }
                        ...
                        sb.AppendLine("\t\tpublic " + staticText + " DataTable GetPageData_RowNumber3(string querystring , string where, int take, int skip, string groupString, string orderString)");
                        sb.AppendLine("\t\t{");
                        sb.AppendLine("\t\t\treturn " + dalName + ".GetPageData_RowNumber3( querystring , where, take, skip, groupString, orderString);");
                        sb.AppendLine("\t\t}");
                        sb.AppendLine("\t\t#endregion");
                        sb.AppendLine("\t\t");
...
GO", tabName, f.ColName, f.Describe);
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        private static string DB_GetObjDescribeSQL(object[] args)
        {
            string tabName = args[0] + "";
            string rel = "";
            StObjField f = args[1] as StObjField;
...
GO", tabName, f.ColName, f.PrientTableName, f_pk.ColName);
        ...
        public static void CreateSQL(string dicPath,string dbName, Dictionary<string, List<StObjField>> fss)
        {
            ...
            tm.Functions.Add("DB_GetObjDescribeSQL", new TemplateFunction(DB_GetObjDescribeSQL));
